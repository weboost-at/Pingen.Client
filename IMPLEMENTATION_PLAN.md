...
- §13 Decision log
- §14 Spec-sync: detecting API drift (post-implementation addendum)
...
The OpenAPI spec is never committed, so the repo instead commits a **fingerprint manifest** of the
contract the client was built against. Checking for drift is then: fetch a fresh spec, re-fingerprint,
diff the fingerprints — and only where they differ does anyone read the spec. Total repo footprint:
two files under `tools/` plus a short CLAUDE.md section.

### 14.1 Footprint

```
tools/
├── SpecSync.cs          # single-file C# app, run via `dotnet run tools/SpecSync.cs -- <mode>`
│                        #   net10 file-based app: no csproj, NOT in the solution, zero packages
│                        #   (System.Text.Json + SHA256 are in-box)
└── spec-manifest.json   # the ONLY committed contract artifact (~12 KB)
```

No CI changes (workflows stay untouched); the check is run by an agent or a human on demand —
typically before a release and whenever Pingen announces changes.
...
```json
{
  ...
  "specVersion": "2.0.0",
  "specUrl": null,
  "generatedFrom": "sha256:<digest of the raw spec file>",
  ...
  "operations": [
    {
      ...
      "kind": "operation",                      // or "webhook-payload" for the 5 /your-webhook-url-for-* docs
      "digest": "a1b2c3d4e5f6",
      "sdk": "LetterService.SendAsync — src/Pingen.Client/Pingen.Client/Deliveries/Letters/LetterService.cs",
      "notes": null                             // deliberate deviations, e.g. "meta_data fields modeled optional (§4 quirk)"
    }
  ]
}
```

- `sdk` is seeded from the §7 matrix — it's what makes a drift report actionable without archaeology.
- `notes` records ACCEPTED disconnects (the §4 "quirks sub-agents must not fix", the LetterMetaData
  optional-fields deviation, unimplemented-on-purpose items). A note means: this divergence is
  known, don't re-litigate it on every check.
- `specUrl`: discover whether Pingen serves the spec JSON at a stable public URL (inspect what
  https://api.pingen.com/documentation loads — do NOT guess). If found, record it and `check` can
  fetch directly; if not, leave `null` and the process is "drop a fresh export into
  `.tmp/swagger-docs.json` first".

### 14.3 Fingerprinting rules (what makes digests stable)

Per operation, digest = first 12 hex chars of SHA-256 over a canonical JSON of the operation object
after:
...
Skip path items with no operations (~180 billing/template stubs) but count them: report the number
so a stub suddenly GAINING operations shows up as ADDED. The five `/your-webhook-url-for-*` docs
are fingerprinted like operations (`kind: "webhook-payload"`) — payload model drift matters too.

### 14.4 Modes and exit codes

```
dotnet run tools/SpecSync.cs -- update [--spec <path|url>]   # (re)generate the manifest, preserving sdk/notes for unchanged ids
dotnet run tools/SpecSync.cs -- check  [--spec <path|url>]   # diff manifest vs fresh spec; exit 0 clean, exit 1 drift
dotnet run tools/SpecSync.cs -- show <operationId> [--spec ] # print the resolved, stripped operation JSON (the §Appendix-B slicer, built in)
```

`--spec` defaults to `.tmp/swagger-docs.json`, falling back to the manifest's `specUrl` when set.
`check` output, grouped and terse — one line per finding:

```
MATCH    47 operations
CHANGED  letters.send      PATCH …/send            → LetterService.SendAsync — …/LetterService.cs
ADDED    letters.restore   POST  …/restore         → not implemented
REMOVED  deliveries.emails.events.image            → EmailService.GetEventImageLocationAsync — …/EmailService.cs
NOTED    2 operations carry accepted-deviation notes (unchanged)
AUTH     unchanged
```

`update` immediately re-runs `check` against the same input and must come back clean — the tool
self-verifies its own idempotence instead of carrying a test project.

### 14.5 The reconciliation process (what an agent does on drift)

1. Obtain a fresh spec (`.tmp/swagger-docs.json`), run `check`.
2. Exit 0 → report "client and spec in sync (N operations, M noted deviations)". Done.
3. Exit 1 → for each CHANGED/ADDED/REMOVED id, and ONLY those: `show <id>`, open the mapped `sdk`
   file, and reconcile — update models/methods/tests for real contract changes; extend the
   `notes` field instead when the divergence is deliberate; implement new operations (this library
   is feature-complete by policy); remove or obsolete SDK methods for removed ones.
4. Run the full test suite, then `update`, confirm `check` is clean.
5. Commit code + manifest together, owner's grammar (§12.2), e.g.
...
### 14.6 CLAUDE.md addition (append verbatim)

```markdown
## Spec sync

The Pingen OpenAPI spec is never committed. tools/spec-manifest.json pins the contract snapshot
this client implements; tools/SpecSync.cs (dotnet run tools/SpecSync.cs -- check|update|show)
compares it against a fresh spec at .tmp/swagger-docs.json. On drift: reconcile ONLY the listed
operations via `show`, update code + tests, run `update`, and commit code and manifest together.
Deliberate divergences live in the manifest's per-operation `notes` — never silently ignore drift.
```

### 14.7 Build order for the implementing agent

One sub-agent, one commit (plus the initial manifest): (1) write `tools/SpecSync.cs` per 14.3/14.4;
(2) try to discover `specUrl` from the public documentation page — record it or leave null;
(3) run `update` against the current `.tmp/swagger-docs.json`; (4) hand-fill `sdk` for all 54
entries from §7 and `notes` from the §4 quirk list + any §13 deviations that surface on the wire;
(5) verify `check` exits 0 and that a cosmetic edit (e.g. editing a `description` in a spec copy)
still exits 0 while deleting an attribute exits 1; (6) commit:
`Added SpecSync.cs and spec-manifest.json, drift against the Pingen spec now surfaces per operation without committing the spec` —
plus the CLAUDE.md section as part of the same commit.
