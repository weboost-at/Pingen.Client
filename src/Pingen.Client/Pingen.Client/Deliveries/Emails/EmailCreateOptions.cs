...
/// <summary>What an email delivery is created from.</summary>
public record EmailCreateOptions
{
    ...
    /// <summary>Whether the email is dispatched as soon as it validates instead of waiting - emails have no send endpoint, so an email left at <c>false</c> is only ever sent through a batch.</summary>
    [JsonPropertyName("auto_send")]
    public required bool AutoSend { get; init; }

    /// <summary>The envelope of the email - all seven members are required once it is set.</summary>
    [JsonPropertyName("meta_data")]
    public EmailMetaData? MetaData { get; init; }

    /// <summary>The preset the email inherits its defaults from.</summary>
    [JsonIgnore]
