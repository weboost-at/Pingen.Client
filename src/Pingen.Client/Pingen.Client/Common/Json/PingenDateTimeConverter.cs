...
public class PingenDateTimeConverter : JsonConverter<DateTimeOffset>
{
    ...
    /// <summary>Reads a timestamp, accepting both the colon-less <c>+0100</c> Pingen writes and the ISO <c>+01:00</c> form.</summary>
    public override DateTimeOffset Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options) =>
        DateTimeOffset.Parse(reader.GetString() ?? "", CultureInfo.InvariantCulture);

    /// <summary>Writes the timestamp in the <see cref="WireFormat"/> the API accepts.</summary>
...
{
    /// <summary>Reads a timestamp that may not have happened yet, mapping the empty string Pingen sends for it to <c>null</c>.</summary>
    public override DateTimeOffset? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options) =>
        reader.GetString() is { Length: > 0 } value ? DateTimeOffset.Parse(value, CultureInfo.InvariantCulture) : null;

    /// <summary>Writes the timestamp in the <see cref="PingenDateTimeConverter.WireFormat"/> the API accepts, or JSON <c>null</c> when it has no value.</summary>
    public override void Write(Utf8JsonWriter writer, DateTimeOffset? value, JsonSerializerOptions options)
    {
